...
} from '@/data/northvietnam/map/places'

const MAP_GTAG_PREFIX = 'northvietnammap'

const CATEGORY_LABEL = CITY_MAP_CATEGORY_LABEL

function northVietnamMapMarkerIcon(place: NorthVietnamMapPlace): google.maps.Icon {
  return cityMapMarkerIcon(place.category as CityMapPlaceCategory, google.maps)
}

/** 方塊內「地圖」按鈕的 `data-event`（自訂請改此字串） */
const MAP_SPOT_GOOGLE_MAPS_BUTTON_EVENT = 'northvietnammap_spot_map'
...
function mapBarMapButtonEvent(place: NorthVietnamMapPlace): string {
  const e = place.mapButtonMapEvent?.trim()
  return e || MAP_SPOT_GOOGLE_MAPS_BUTTON_EVENT
}

function mapBarMapButtonLabel(place: NorthVietnamMapPlace): string {
...
export default function NorthVietnamMapClient() {
  ...
  const [categoryOn, setCategoryOn] =
    useState<Record<NorthVietnamPlaceCategory, boolean>>(DEFAULT_CITY_MAP_CATEGORY_ON)
  ...
  const filteredPlaces = useMemo(
    () => northVietnamMapPlaces.filter((p) => categoryOn[p.category]),
    [categoryOn],
  )
  ...
  const northVietnamListSections = useMemo(
    () => [
      {
        places: spotPlaces,
    ...
    (places: NorthVietnamMapPlace[]) => {
      ...
      for (const p of places) {
        const marker = new google.maps.Marker({
          ...
          icon: northVietnamMapMarkerIcon(p),
      ...
      >
        <div className={styles.topBar}>
          ...
          <div className={styles.introBlock}>
            <div
              className="tabs"
              id="northVietnamMapCategoryToggles"
              role="group"
        ...
        <div className={styles.splitWrap}>
          ...
          <aside className={styles.listColumn} aria-label="地點列表（桌機）">
            <div ref={desktopListScrollRef} className={styles.desktopListScroll}>
              {northVietnamListSections.map((s) =>
                s.places.length > 0 ? (
                  <section
                    key={`desk-${s.aria}`}
