...
export default function NewExpenseForm({
  ...
  const total = useMemo(() => num(amount), [amount])
  ...
  const computedSplit = useMemo(() => {
    ...
    ids.forEach((id) => {
      exclusive[id] = num(exclusiveAmounts[id])
      if (lockedSharedIds.has(id)) shared[id] = num(sharedOverrides[id])
    })
    unlocked.forEach((id, idx) => { shared[id] = base + (idx < rem ? 1 : 0) })
            ...
            <div className="modal" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="modal-body">
                <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', justifyContent: 'space-between' }}>
                  ...
                  <div style={{ fontSize: 13, color: payerRemaining === 0 ? '#059669' : '#b91c1c' }}>尚有 ${payerRemaining} 未分配</div>
                </div>
                <div style={{ fontSize: 12, color: '#6b7280', marginTop: 6 }}>勾選付款者後會自動平均，修改金額時會重新計算剩餘。</div>
                      ...
                      <label key={m.id} style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: 14 }}>
                        ...
                        {checked ? (
                          <input
                            ...
                            inputMode="numeric"
                            value={payerAmounts[m.id] ?? (checked ? String(computedPayerAmounts[m.id] ?? '') : '')}
                            onChange={(e) => {
                              ...
                              setPayerAmounts((prev) => {
                                ...
                                if (ids.length >= 3) {
                                  const manualCount = ids.filter((id) => next[id] !== undefined && next[id] !== '').length
                        ...
                        {checked ? (
                          <>
                            <input
                              ...
                              inputMode="numeric"
                              placeholder="平均"
                              value={sharedLocked ? (sharedOverrides[m.id] ?? '') : String(computedSplit.shared[m.id] ?? 0)}
                              onChange={(e) => {
                                const v = e.target.value
                                setSharedOverrides((prev) => {
                                  const next = { ...prev, [m.id]: v }
                                  const ids = [...effectiveSplitterIds].sort()
                                  if (ids.length === 2) { const otherId = ids[0] === m.id ? ids[1] : ids[0]; delete next[otherId] }
                                  return next
                                })
                            ...
                            />
                            <div style={{ width: 70, textAlign: 'right', fontWeight: 800 }}>{computedSplit.totalByMember[m.id] ?? 0}</div>
                          </>
