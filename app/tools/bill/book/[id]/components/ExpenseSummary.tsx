...
export default function ExpenseSummary({
  ...
  const [settlementMode, setSettlementMode] = useState<SettlementMode>('least') // least=最少轉帳, simple=集中結算
  ...
  const compute = (mode: SettlementMode): { total: number; balances: BalanceRow[]; settlements: SettlementRow[] } => {
    if (!members.length) {
              ...
              <div style={{ marginTop: 12 }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  {balances.map((b) => {
                    ...
                    const color = positive ? '#dc2626' : '#059669'  /* 正值紅、負值綠 */
                    return (
                      <div
                        key={b.id}
                        style={{
