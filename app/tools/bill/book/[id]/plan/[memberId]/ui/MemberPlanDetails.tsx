...
export default function MemberPlanDetails({
  ...
  const downloadPdf = () => {
    ...
    const rowsPaid = paidLines
      .map((x) => `<tr><td>${esc(x.date || '')}</td><td>${esc(x.title)}</td><td class="num">${x.amount}</td></tr>`)
      .join('')
    const rowsSpend = shareLines
      .map((x) => `<tr><td>${esc(x.date || '')}</td><td>${esc(x.title)}</td><td class="num">${x.amount}</td></tr>`)
      .join('')

    const html = `<!doctype html><html><head><meta charset="utf-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/><title>${esc(memberName)} 消費明細</title><style>:root{--brand:#1f7a8c;--ink:#0f172a;--muted:#64748b;--bg:#f3f6f9;}body{font-family:ui-sans-serif,system-ui,"Noto Sans TC",Arial;margin:0;background:var(--bg);color:var(--ink);}.wrap{max-width:820px;margin:24px auto;padding:0 16px;}.head{display:flex;justify-content:space-between;align-items:flex-start;padding:14px 16px;background:#fff;border:2px solid rgba(31,122,140,.35);border-radius:16px;}.title{font-weight:900;font-size:18px;}.sub{color:var(--muted);font-size:13px;margin-top:4px;}.card{background:#fff;border:1px solid rgba(0,0,0,.06);border-radius:14px;padding:14px 16px;margin-top:14px;}h3{margin:0 0 10px;font-size:15px;}table{width:100%;border-collapse:collapse;font-size:13px;}th,td{padding:10px;border-bottom:1px solid rgba(0,0,0,.06);}.total{font-weight:900;background:#f1f5f9;}@media print{body{background:#fff;}}</style></head><body><div class="wrap"><div class="head"><div><div class="title">${esc(memberName)} 消費明細</div><div class="sub">${esc(bookName)}</div></div></div><div class="card"><h3>消費</h3><table><thead><tr><th>日期</th><th>項目</th><th>金額</th></tr></thead><tbody>${rowsSpend || '<tr><td colspan="3">沒有消費紀錄</td></tr>'}<tr class="total"><td></td><td>總額</td><td>${shareTotal}</td></tr></tbody></table></div><div class="card"><h3>已付</h3><table><thead><tr><th>日期</th><th>項目</th><th>金額</th></tr></thead><tbody>${rowsPaid || '<tr><td colspan="3">沒有付款紀錄</td></tr>'}<tr class="total"><td></td><td>總額</td><td>${paidTotal}</td></tr></tbody></table></div></div><script>window.onload=()=>{setTimeout(()=>window.print(),50);};</script></body></html>`
    const iframe = document.createElement('iframe')
    iframe.style.cssText = 'position:fixed;right:0;bottom:0;width:0;height:0;border:0'
    iframe.srcdoc = html
    document.body.appendChild(iframe)
    iframe.onload = () => {
      try { iframe.contentWindow?.print() } finally { setTimeout(() => iframe.remove(), 1000) }
    }
  }
      ...
      {!loading && !error ? (
        <>
          ...
          <div style={{ marginTop: 10 }}>
            <button className="pill-link" style={{ border: 'none', cursor: 'pointer' }} type="button" onClick={() => setShowDownload(true)} data-event="downloadbill">下載</button>
          </div>

          <div style={{ marginTop: 14 }}>
            <div style={{ fontWeight: 800, color: '#16324f', marginBottom: 6 }}>消費</div>
            {shareLines.length === 0 ? <div style={{ color: '#94a3b8', fontSize: 13 }}>沒有消費紀錄</div> : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                {shareLines.map((x) => (
                  <div key={`s-${x.expenseId}-${x.amount}`} style={{ display: 'flex', justifyContent: 'space-between', gap: 12 }}>
                    ...
                    <div style={{ fontWeight: 900, color: '#16324f', whiteSpace: 'nowrap', fontVariantNumeric: 'tabular-nums' }}>{x.amount}</div>
                  </div>
                ))}
              </div>
            )}
          ...
          <div style={{ marginTop: 14 }}>
            ...
            {paidLines.length === 0 ? <div style={{ color: '#94a3b8', fontSize: 13 }}>沒有付款紀錄</div> : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                {paidLines.map((x) => (
          ...
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
              <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                <button className="btn secondary" style={{ width: 160 }} type="button" onClick={downloadExcel} data-event="downloadbillexcel">Excel（.xlsx）</button>
                <button className="btn secondary" style={{ width: 160 }} type="button" onClick={downloadPdf} data-event="downloadbillpdf">PDF</button>
              </div>
              <div style={{ color: '#64748b', fontSize: 13, marginTop: 10 }}>PDF 會開啟列印視窗，選「另存為 PDF」即可下載。</div>
            </div>
