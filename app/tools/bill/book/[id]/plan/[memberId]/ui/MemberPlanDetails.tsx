...
export default function MemberPlanDetails({
  ...
  const downloadExcel = async () => {
    ...
    supabase.from('download_logs').insert({ book_id: bookId, member_id: memberId, format: 'excel' }).then(() => {}, (err) => console.error('download_logs insert failed:', err))
  }
...
<body>
  ...
  <script>window.onload=function(){setTimeout(function(){window.print()},80)};</script>
    ...
    supabase.from('download_logs').insert({ book_id: bookId, member_id: memberId, format: 'pdf' }).then(() => {}, (err) => console.error('download_logs insert failed:', err))
  }
